...
int rand(){
    TIMER->H_Counter_Snap = 1;
    int a = TIMER->H_Counter_Snap;
    a += TIMER->L_Counter_Snap;
    printf("%d\n",a);
    return a;
}
...
void printCell(int pos_row, int pos_col, int row_num, int col_num){
    ...
    for (int row = 0; row < border_size; row++){
        for (int col = 0; col < col_num; col++){
            c_vga[pos_row + row][pos_col + col] = light_border_color;
        }
    }
    ...
    for (int row = border_size; row < row_num - border_size; row++){
        for (int col = border_size; col < col_num - border_size; col++){
            c_vga[pos_row + row][pos_col + col] = base_color;
        }
    }
    ...
    for (int row = row_num - border_size; row < row_num; row++){
        for (int col = 0; col < col_num; col++){
            c_vga[pos_row + row][pos_col + col] = dark_border_color;
        }
    }
    for (int row = border_size; row < row_num; row++){
...
void printBoard(struct BOARD *board, char *end, char *win){
    for (int row = 0; row < 9; row++){
        ...
        for (int column = 0; column < 9; column++){
            ...
            printCell(pos_buffer_row * 4 - 4, pos_buffer_col * 4 - 4, 12, 12);
    ...
    printCell(45 * 4 - 4, 28 * 4 - 4, 12, 24);
    c_buffer[28][45] = 'O';
    c_buffer[29][45] = 'P';
    c_buffer[30][45] = 'E';
    c_buffer[31][45] = 'N';

    printCell(45 * 4 - 4, 35 * 4 - 4, 12, 28);
    c_buffer[35][45] = 'z' + 5;
    c_buffer[36][45] = 'F';
    c_buffer[37][45] = 'L';
    c_buffer[38][45] = 'A';
...
void parseMouse(struct BOARD *board, char *status, char *end, char *win){
    if(!clicked)
        return;
    clicked = 0;
    if(G_mouse_X/8 < 132 && G_mouse_X/8 > 108 && G_mouse_Y/8 < 188  && G_mouse_Y/8 > 176 ){
        *status = 0;
        return;
    }
    if(G_mouse_X/8 < 164 && G_mouse_X/8 > 136 && G_mouse_Y/8 < 188 && G_mouse_Y/8 > 176){
        *status = 1;
        return;
    }
    if(G_mouse_X/8 < 204 && G_mouse_X/8 > 168 && G_mouse_Y/8 < 188 && G_mouse_Y/8 > 176){
        restart(COLSIZE, ROWSIZE, board, end, win);
        return;
    }
    if(!(*end)){
        //note 104 is the chosen upper edge
        int cnt_x = (G_mouse_X/8-104)/12;
        int cnt_y = (G_mouse_Y/8-64)/12;
